import React, { Component, useEffect, useState } from 'react';

import { Image, FlatList, View, Text, TouchableHighlight, Button, TouchableOpacity } from 'react-native';
import { ListItem, Avatar, Rating, AirbnbRating, SearchBar } from 'react-native-elements'
import MaterialCommunityIcons from 'react-native-vector-icons/MaterialCommunityIcons'
import {JobCard} from '../../components/WorkerComponents/WorkerComponents';

import { NavigationContainer, useNavigation, useTheme } from '@react-navigation/native';
import { createStackNavigator } from '@react-navigation/stack';

import { ScrollView } from 'react-native-gesture-handler';



 function Workers({navigation, route}) {
   const { colors } = useTheme();
/////////////////////////////////////////////////////////// Styling components ////////////////////////////////////////////////////////

const view ={
  backgroundColor: colors.background
}

const cardStyle = {
        margin : 10,
        padding: 16,
        color: colors.text,
        backgroundColor: colors.card,
    };

const description= {
        flex: 2,
        flexDirection: 'row',
        margin: 10,
        marginLeft: 40,
        color: colors.text
        //Made the left 40 so it is not so close to the edge
    };

    //Button customization
const rating= {
        flex:1,
        //flexDirection: 'row',
        margin: 10
    };

const categoryButtons= {
        padding : 15,
        margin: 3,
        backgroundColor: colors.card,

        borderRadius: 15,
    };

const categoryText = {
  color: colors.text
};
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

//--------------------------------------------------This is a dictionary of job categories ----------------------------------------------------------------
  let jobCategories =  ["Favourites"];

   //in react navigation the route returns undefined if the object was never called, that is why we test whether the object is undefined "Meaning never called"
   // doing this will test whether we arrive through route of the browse screen or through the entry point of when the app is first opened on the jobs screen
   // If we do arrive through the route, being the browse screen we then concat the array onto our jobCategories array to use in rendering the buttons
   if (typeof route.params != 'undefined'){
     jobCategories= jobCategories.concat(route.params.item.skill);
   }

//---------------------------------------------------------------------------------------------------------------------------------------------------------------------

    return(
          {/*Making use of flat list because it is more efficient in rendering large amounts of data*/},
            {/* The header component is used for performance reasons */},
          <FlatList
              style = {view}
                showsVerticalScrollIndicator={false}
                ListHeaderComponent = {
                  <>
                        <SearchBar
                            lightMode
                            round
                            placeholder="Search"
                            borderless
                            containerStyle = {{ backgroundColor: colors.card, borderTopColor: colors.border, borderBottomColor: colors.border  }}
                          inputContainerStyle = {{ height: 25,  borderColor: colors.border, backgroundColor: colors.background }}
                        />

                        <ScrollView
                            horizontal
                            showsHorizontalScrollIndicator={false}
                        >
                            {
                                jobCategories.map((j, i) => {
                                    return(
                                        <TouchableOpacity
                                          style = {categoryButtons}
                                          key ={i}
                                        >
                                          <Text style = {categoryText} >{j}</Text>
                                        </TouchableOpacity>

                                    )
                                })
                            }

                        </ScrollView>
                  </>
              }
              data = {[
                        {

                        title: "Clown",
                        skills: ["Funny", "Entertainment", "Events"],
                        done: false,
                        budget: "R1000",
                        details: "I am looking for a clown who will be coming to my brothers birthday party. And he needs to be verry funny and must have a red nose but it must not look like IT,yk the clown from the horror movie coz even though it wasn't scary it was just creepy coz if you think about it it's basically a pedo clown and that's weird ",
                        dateSet: "20/12/2019", // dd/mm/yyyy
                        promoted: true,
                        User : {
                            name: 'Justin Botha',
                            time: "2021/02/05",
                            location: "Krugersdorp",
                            description: "I will sing happy birthday to you out of my nostrils",
                            avatar: "data:image/png;base64,",
                            available: false,
                            travelDistance: "1000km",
                            trusters: 20000,
                            reporters: 0,
                            totalJobs: 2120,
                            skills: [ { key: "Audio"},
                                    {key:"Arts"},
                                    { key: "skilles"}
                                  ]

                        }},
                        {
                        title: "Photography",
                        skills: ["Arts"],
                        done: true,
                        budget: "R100",
                        details: "I need a photographer for my sisters wedding.",
                        dateSet: "20/12/2019", // dd/mm/yyyy
                        dateCompleted: "25/12/2019", // dd/mm/yyyy
                        promoted: false,
                        User:{
                            name: 'Chris Jackson',
                            time: "2021/02/05",
                            location: "North Pretoria",
                            description: "I want somone to help me design and set up my personal protfolio website. I need it to be done with profisedf aisoo aodf sjdf oapRKM KSDKF KEKSDKF ifsndifjwemr poaeoj wkefoo woe woejr mmvorw mfelpk peofw peomfjdn ejwfj j",
                            avatar: "https://images.unsplash.com/photo-1497802176320-541c8e8de98d?&w=1600&h=900&fit=crop&crop=entropy&q=300",
                            available: true,
                            travelDistance: "5km",
                            trusters: 341,
                            reporters: 1,
                            totalJobs: 56,
                          skills: [ {key: "Computer"},
                                    {key: "Technology"} ]
                        }},
                        {
                        title: "Photography",
                        skills: ["Arts"],
                        done: true,
                        budget: "R100",
                        details: "I need a photographer for my sisters wedding.",
                        dateSet: "20/12/2019", // dd/mm/yyyy
                        dateCompleted: "25/12/2019", // dd/mm/yyyy
                        promoted: false,
                        User: {
                            name: "Christopher Moon",
                            time: "2021/06/01",
                            location: "Los Angeles, CA",
                            description:"This is the description lorem s, I want someone who will be able to assist me with my taxas because I have to submit it next week",
                            avatar: "http://i.pravatar.cc/100?id=skater",
                            available: true,
                            travelDistance: "10km",
                            trusters: 1,
                            reporters: 20,
                            totalJobs: 304,
                            skills: []
                        }},
                    ]}
                    renderItem={({item}) =>
                      <TouchableHighlight
                        onPress = {() => {
                          const client = item.User;
                          navigation.navigate('WorkerJobDetailsScreen', { item, client})}}
                        underlayColor = { colors.background }
                      >
                         <JobCard
                            Data = { item }
                            User = { item.User }
                            rating = {false}
                            promoted = { item.promoted }
                        />
                      </TouchableHighlight>
                    }
            //The key extractor is specify a key property for each item
                    keyExtractor={(item, index) => index.toString()}
            />
    );
  }

export default React.memo(Workers);
